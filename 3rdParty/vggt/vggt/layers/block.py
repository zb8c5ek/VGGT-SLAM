...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, return_kq=False) -> Tensor:
        def attn_residual_func(x: Tensor, pos=None, return_kq=False) -> Tensor:
            if return_kq:
                out, k, q = self.attn(self.norm1(x), pos=pos, return_kq=True)
                return self.ls1(out), k, q
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            if return_kq:
                x_new, k, q = attn_residual_func(x, pos=pos, return_kq=True)
                x = x + x_new
